...
option_list <- list(
    make_option(c("-r", "--RepeatRegions"), type="character", default="/data1/greenbab/users/ahunos/apps/workflows/methylation_workflows/DNAme_Ref_LINE1/outputs/SQuireRepeatsValidReadCounts_Aggregated_FWD_REV.fst", 
...
L1MD <- ovlapsStatsDTFull[clade == "L1" & grepl("L1Md",consensus)]
...
plt_methyl <- ggboxplot(ovlapsStatsDTFull[class %in% c("SINE", "scRNA", "LINE")], x = "class", y = "fracMethyl.mean", palette = "jco")+
...
plt_methylL1 <- ggplot(data = ovlapsStatsDTFull[clade == "L1"], aes(x=consensus, y=fracMethyl.mean)) + 
geom_boxplot() + 
